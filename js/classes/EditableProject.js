class EditableProject {
    ...
    toggleEditOn() {
        if (!this.inputEl) {
            ...
            $.each(this.timeTracker.getCategories(), function(editableProject) {
                return function(categoryIndex, category) {
                    const projectList = category.getProjects();
                    if (projectList.length > 0) {
                        // Show a category header
                        editableProject.inputEl.append(`<optgroup label="${Utils.escapeHTML(category.getName())}">`);
                        $.each(projectList, function(projectIndex, project) {
                        ...
                        editableProject.inputEl.append(`</optgroup>`);
                    }
                };
            }(this));
